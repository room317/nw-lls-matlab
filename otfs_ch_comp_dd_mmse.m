function [rx_sym_dd_ndft_eq, ch_est_dd_ndft, ch_est_tf_ndft] = otfs_ch_comp_dd_mmse(rx_sym_dd_ndft, rx_sym_tf_ndft, idx_pilot_sym, num, noise_var, chest_option, ch_dd_real)

% offset pilot (to prevent data penetration)
pilot_offset = [20 1]; % [20 1];

% demap pilot symbols
pilot_sym = rx_sym_dd_ndft(idx_pilot_sym(1)+1+pilot_offset(1):idx_pilot_sym(1)+num.num_pilot_delay-pilot_offset(1), idx_pilot_sym(2)+1+pilot_offset(2):idx_pilot_sym(2)+num.num_pilot_doppler-pilot_offset(2));
