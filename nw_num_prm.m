function nw_num = nw_num_prm(idx_bandwidth, eq_dd)
...
idx_pilot_ofdmsym = [4 11]; % [8 9];         % pilot symbol index (should be scalar or vector, all elements should be between 1 and 14)
idx_data_ofdmsym(idx_pilot_ofdmsym) = [];
num_data_ofdmsym_per_subframe = length(idx_data_ofdmsym); % 14;          % number of ofdm symbols per subframe (data)
num_pilot_ofdmsym_per_subframe = length(idx_pilot_ofdmsym); % 0;          % number of ofdm symbols per subframe (data)
num_ofdmsym_per_subframe = 14;      % number of ofdm symbols per subframe (pilot + data)
...
% otfs pilot setup
%     set number of otfs pilot symbols be equal or similar to the number of
%     ofdm pilot symbols
%     ex) 600*2 and 86*14
if eq_dd
    len_pilot_otfssym = 14;         % length of pilot rb to otfs symbols (set even number)
    len_pilot_subc = 86;            % length of pilot rb to subcarriers (set even number)
    len_rb_sym_user = (ndft * num_ofdmsym_per_subframe) - (len_pilot_otfssym * len_pilot_subc);          % number of data symbols per user
    
    % output
    nw_num.len_pilot_otfssym = len_pilot_otfssym;
    nw_num.len_pilot_subc = len_pilot_subc;
end
