...
if test_option.gpu_flag
    ch_coeff_mat = gpuArray(zeros(nfft, nfft, 1, npath));
    ch_mat_t = gpuArray(zeros(nfft, nfft, nsym));
else
    ch_coeff_mat = zeros(nfft, nfft, 1, npath);
    ch_mat_t = zeros(nfft, nfft, nsym);
end

% reshape path gain matrix
if test_option.gpu_flag
    ch_pg_reshape = gpuArray(reshape(ch_pg, nfft+ncp, 1, nsym, npath));
else
    ch_pg_reshape = reshape(ch_pg, nfft+ncp, 1, nsym, npath);
end
ch_pg_reshape(1:ncp, :, :, :) = [];
...
for idx_path = 1:npath
    ...
    ch_coeff_mat(:, :, 1, idx_path) = toeplitz(ch_coeff_col, ch_coeff_row);
end
