...
%  - tx_sym_data_slot: slot data per user (vector)
%  - tx_sym_pilot_slot: slot pilot per user (matrix)

function [rx_sym_data_slot, rx_sym_pilot_slot] = ofdm_sym_demap(rx_sym_rbs, num)
...
rx_sym_pilot_slot = zeros(num.num_subc_pilot_usr, num.num_ofdmsym_pilot_usr);
for i = 1:num.num_ofdmsym_pilot_usr
    rx_sym_pilot_slot(:, i) =  rx_sym_rbs(num.idx_subc_pilot_usr(:, i), num.idx_ofdmsym_pilot_usr(i));
end
