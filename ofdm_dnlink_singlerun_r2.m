...
function [pkt_error, tx_papr, ch_mse, sym_err_var] = ofdm_dnlink_singlerun_r2(sim, cc, rm, num, snr_db, ch, chest_option, cheq_option, test_option)
...
for idx_usr = 1:num.num_usr
    % generate transmit symbol per user
    [tx_bit_usr, tx_sym_usr, tx_sym_rbs_usr] = gen_tx_qamsym_usr_r1(sim, cc, rm, num, tx_crc, turbo_enc);
    tx_bit(:, idx_usr) = tx_bit_usr;
    tx_sym(:, :, idx_usr) = tx_sym_usr;
    tx_sym_rbs(:, :, :, idx_usr) = tx_sym_rbs_usr;
    ...
    for idx_usrfrm = 1:sum(rm.num_usrfrm_cb)
        tx_sym_bw(list_subc_usr, list_ofdmsym_usr, idx_usrfrm, idx_usr) = tx_sym_rbs_usr(:, :, idx_usrfrm);
    end
end
