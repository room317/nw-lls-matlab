...
function [tx_sym_rbs_dd, tx_sym_pilot_usrfrm] = otfs_sym_map_r2(tx_sym_data_usrfrm, num, test_option)
...
if test_option.otfs_map_plan == 1 || test_option.otfs_map_plan == 2 || test_option.otfs_map_plan == 3   % use impulse pilot
    ...
    tx_sym_pilot_usrfrm = zeros(num.num_delay_pilot_usr, num.num_doppler_pilot_usr);
    tx_sym_pilot_usrfrm(idx_delay_pilot_usr, idx_doppler_pilot_usr) = sqrt(pwr_pilot);     % boosts pilots
...
%     tx_sym_pilot_guard_slot(idx_pilot_seq(1)+1:idx_pilot_seq(1)+length(test_option.pilot_spread_seq), idx_pilot_seq(2)+1) = sqrt(pwr_pilot)*test_option.pilot_spread_seq;
    tx_sym_pilot_usrfrm = zeros(num.num_delay_pilot_usr, num.num_doppler_pilot_usr);
    tx_sym_pilot_usrfrm(idx_delay_pilot_usr-ceil(length(test_option.otfs_pilot_spread_seq)/2)+1: ...
...
%     tx_sym_pilot_guard_slot(idx_pilot_seq(1)+1:idx_pilot_seq(1)+length(test_option.pilot_spread_seq), idx_pilot_seq(2)+1) = sqrt(pwr_pilot)*test_option.pilot_spread_seq;
    tx_sym_pilot_usrfrm = zeros(num.num_delay_pilot_usr, num.num_doppler_pilot_usr);
    tx_sym_pilot_usrfrm(idx_delay_pilot_usr-ceil(length(test_option.otfs_pilot_seq_ones)/2)+1: ...
...
tx_sym_data1_usrfrm = sqrt(pwr_data)*reshape(tx_sym_data_usrfrm(1:num.num_delay_data_usr*num.num_doppler_usr), num.num_delay_data_usr, []);
tx_sym_data2_usrfrm = sqrt(pwr_data)*reshape(tx_sym_data_usrfrm(num.num_delay_data_usr*num.num_doppler_usr+1:end), num.num_delay_pilot_usr, []);

% fprintf('data1:%6.3f\n', sqrt(mean(abs(tx_sym_data1_usrfrm).^2, 'all')))
% fprintf('data2:%6.3f\n', sqrt(mean(abs(tx_sym_data2_usrfrm).^2, 'all')))

% map resource block
tx_sym_map_basic = [ ...
    tx_sym_pilot_usrfrm, tx_sym_data2_usrfrm;
    tx_sym_data1_usrfrm];
...
% assignin('base', 'tx_sym_pilot_usrfrm', tx_sym_pilot_usrfrm);
